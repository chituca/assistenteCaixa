...
module.exports = {
    ...
    getMegaSena: function (result) {
        ...
        if(ganhadores === 0) {
            ...
            return retorno = cabecalho+"<break time=\"1s\"/>o prêmio <emphasis level=\"moderate\">acumulou</emphasis> e a estimativa para o próximo concurso, em "+dataProximo+
    ...
    if(ganhadores === 0) {
     ...
     getDuplaSena: function (result) {
        ...
        " da Dupla-Sena <break time=\"1s\"/>os números sorteados do <say-as interpret-as=\"ordinal\">1</say-as>º sorteio são: " +
        "<say-as interpret-as=\"cardinal\">"+sorteados[0]+"</say-as>,"+
        "<say-as interpret-as=\"cardinal\">"+sorteados[1]+"</say-as>,"+
        "<say-as interpret-as=\"cardinal\">"+sorteados[2]+"</say-as>,"+
        "<say-as interpret-as=\"cardinal\">"+sorteados[3]+"</say-as>,"+
    ...
    //Dia de Sorte
    getDiaSorte: function (result) {
        let retorno;
        let concurso = result.resultado.NU_CONCURSO;
        let ganhadores = result.resultado.QT_GANHADOR_FAIXA_1;
        let sorteados = result.resultado.DE_DEZENA_SORTEADA.split('-').sort();
        let dataSorteio = util.formataData(result.resultado.DT_APURACAO_SORTEIO);
        let cabecalho = "<speak>hoje pode ser o seu dia! para o concurso <say-as interpret-as=\"cardinal\">"+concurso+
        "</say-as><break time=\"1s\"/> os números sorteados do <say-as interpret-as=\"ordinal\">1</say-as>º sorteio são: " +
        "<say-as interpret-as=\"cardinal\">"+sorteados[0]+"</say-as>,"+
        "<say-as interpret-as=\"cardinal\">"+sorteados[1]+"</say-as>,"+
        "<say-as interpret-as=\"cardinal\">"+sorteados[2]+"</say-as>,"+
        "<say-as interpret-as=\"cardinal\">"+sorteados[3]+"</say-as>,"+
        ...
        "<say-as interpret-as=\"cardinal\">"+sorteados[6]+"</say-as>.";

        if(ganhadores === 0) {
            var estimativa = util.formataReal(result.resultado.VR_ESTIMATIVA_PREMIO_FAIXA_1);
            var acumulado = util.formataReal(result.resultado.VR_ACUMULADO_FAIXA_1);
            return retorno = cabecalho+"<break time=\"1s\"/>o prêmio <emphasis level=\"moderate\">acumulou</emphasis> e a estimativa para o próximo concurso, é de "+ 
            estimativa + " <break time=\"1s\"/> e o valor acumulado foi "+acumulado+".</speak>";
        
            } else {
            var premio = util.formataReal(result.resultado.VR_RATEIO_FAIXA_1);
            var apostasTexto = '';
                if(ganhadores > 1){
                    apostasTexto = "apostas foram premiadas";
                } else {
    ...
    getLoteca: function (result) {
        let retorno;
        let concurso = result.resultado.co_concurso;
        let ganhadores = result.resultado.qt_ganhador_faixa1;
        let dataSorteio = util.formataData(result.resultado.dt_apuracao);
        let cabecalho = "<speak>O concurso "+concurso+" da Loteca, sorteado em "+dataSorteio+", ";

        if(ganhadores === 0) {
            var estimativa = util.formataReal(result.resultado.VR_ESTIMATIVA_PREMIO_FAIXA_1);
            var acumulado = util.formataReal(result.resultado.VR_ACUMULADO_FAIXA_1);
            return retorno = cabecalho+"<emphasis level=\"moderate\">Acumulou!</emphasis> <break time=\"1s\"/> </speak>"+
            "para detalhar o resultado acesse: http://www.loterias.caixa.gov.br/wps/portal/loterias/landing/loteca";
            ...
            return retorno = cabecalho+"<break time=\"1s\"/> teve <say-as interpret-as=\"cardinal\"> "+ganhadores+
            "</say-as>"+apostasTexto+" com valor de "+premio+"</speak>";    
            }

    },

    getLotogol: function (result) {

        let concurso = result.resultado.CO_CONCURSO;
        let dataSorteio = util.formataData(result.resultado.dt_apuracao);
        let retorno = "<speak>O resultado do concurso "+concurso+" da Lotogol, sorteado em  "+dataSorteio+", "+
            "pode ser acessado em: http://www.loterias.caixa.gov.br/wps/portal/loterias/landing/lotogol";
